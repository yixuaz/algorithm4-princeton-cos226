...
public class DocumentSearchTest {
    ...
    private int expect(String document, String query) {
        int m = document.length(), n = query.length(), len = m - n;
        char[] ts = query.toCharArray();

        int idx = -1, minLen = Integer.MAX_VALUE;
        while (idx < len) {
            for (int tidx = 0; tidx < n; tidx++) {
                idx = document.indexOf(ts[tidx], idx + 1);
                if (idx == -1) return minLen == Integer.MAX_VALUE ? -1 : minLen;
            }
            int end = ++idx;
            for (int tidx = n - 1; tidx >= 0; tidx--) {
                idx = document.lastIndexOf(ts[tidx], idx - 1);
            }
    ...
    private int expect2(String document, String query) {
        int m = query.length(), n = document.length();
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                if (query.charAt(i - 1) == document.charAt(j - 1)) {
                    dp[i][j] = dp[i - 1][j - 1];
                } else {
                    dp[i][j] = dp[i][j - 1];
                }
